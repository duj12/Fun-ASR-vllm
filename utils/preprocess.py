...
def insert_space_between_mandarin(text):
    ...
    out_text = text[0]
    for i in range(1, len(text)):
        if is_mandarin(text[i-1]) and not is_mandarin(text[i]):
            out_text += " "
...
def write_mandarin_only_text(train_text, file, char_file1, char_file2):
    """write Mandarin text data"""
    fin = open(train_text, 'r', encoding='utf-8')
    ...
    with open(file, "w", encoding='utf-8') as fp:
        for idx, content in enumerate(fin):
            ...
            name = content[0]
            text = " ".join(content[1:])
            text = insert_space_between_mandarin(text)
...
def write_bpe_train_text(train_text, file):
...
if __name__ == "__main__":
    ...
    train_text_w_space = sys.argv[2]
    mandarin_char = sys.argv[3]
    bpe_nlsyms = sys.argv[4]
    bpe_train = sys.argv[5]
